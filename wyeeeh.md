...
**区块空间宪章 Blockspace Charters**是一种框架，为 Optimism 生态系统中的区块空间建立标准、管理政策和预先承诺，确保安全性、正常运行时间并符合链的法则。
...
- 通过治理投票：章程中引用的三个 TOML 文件（标准配置参数、标准配置角色和标准版本 standard config params, standard config roles, and standard versions）需要治理投票才能更改。
...
**The Standard Rollup Charter 标准汇总章程**是区块空间章程，它定义了成为标准链（highest-security flagship blockspace）的要求。标准 Rollup 章程确保链遵守必要的技术和操作基准，以保持超级链中最高的安全性、兼容性和合规性标准。
补充细则：[GitHub: Standard Rollup Charter ](https://github.com/ethereum-optimism/OPerating-manual/blob/feat/standard-rollup-charter/Standard%20Rollup%20Charter.md)

1. Onchain criteria 链上标准
标准汇总的链上标准包括两个部分： 版本检查和配置检查 。
   - 版本控制 Version validation：链必须部署经治理批准的、最新的 OP Stack 版本。
   - 配置参数：区块链参数（例如区块时间和 gas 计量）必须符合治理批准的规范。针对该链的特权管理角色必须符合安全理事会的要求。
2. Off-chain criteria  链下标准
Optimism 基金会执行链下检查来验证合规性。
   - ChainID 检查 ：确保 Chain ID 是唯一的，并且不会与预先存在的 EVM 链冲突。
   - Gas 限制检查 ：确保根据 OP Stack 性能测试的当前状态，将链的初始 GasLimit 配置为合理的值。
   - 安全监控 ：确保每个链都部署了某些标准监控工具。
   - 管理者/服务者验证 ：在社交层面验证该链确实是由相关方部署和操作的，并且没有被冒充。
3. History Integrity Checks 历史完整性检查
